...
- [ ] Add `clearCurrentTaskId()` session helper.
- [ ] Implement `startNewTaskAction()` to clear current task and redirect.
- [ ] Implement `selectProjectAction()` to validate project id, set current project, clear current task, and redirect.
- [ ] Implement `selectTaskAction()` to validate task id, set current task/project, and redirect.
- [ ] Add regression tests for valid and invalid action paths.
...
- [ ] Convert sidebar new-task, project rows, and task rows to real forms/buttons.
- [ ] Replace fake empty task titles with explicit empty-state copy.
- [ ] Convert entry chips and task suggestions to quick prompt submit forms.
- [ ] Add focused CSS for forms/buttons without changing layout architecture.
- [ ] Add render tests for hidden payloads and empty-state behavior.
...
- [ ] Update manual alpha checklist for sidebar navigation and quick prompts.
...
- [ ] Mark Stage 33 implemented and summarize completed scope.
