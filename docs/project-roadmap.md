...
- Task Intent Routing / AI Follow-ups v0：Stage59 已完成设计，建议作为当前 Web/agent 体验小阶段优先实现：普通聊天不显示推荐追问；LP 复杂任务才显示基于上下文生成的推荐追问；LP task 内每次输入先由 AI intent router 判定普通问答、继续执行、新复杂任务或澄清，deterministic router 只作为 no-key 测试和安全兜底，避免把所有输入都当作 agent continuation。
