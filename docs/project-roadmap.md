...
- Post-V1 backlog prioritization v0：Stage 49 已完成 docs-only backlog scoring，默认下一阶段为 Stage 51 MCP Management Surface v0 Spec Kickoff；Stage 51 design 和 implementation plan 已写入，但本 roadmap 暂不把 Stage 51 标为已完成，completion closeout 后再处理完成状态；Stage 48 保持 blocker 条件触发，Stage 50 保持 browser platform / visual baseline 可选规划路径。
...
- Stage 54 之外的真实 MCP SDK / remote MCP server adapter、write tools、MCP worker execution、secret storage、auth/RBAC 和 raw MCP output 通道；Stage 54 只能实现 Stage 51 定义的安全 Web 管理面。
...
**状态：** design 和 implementation plan 已写入；当前未标记为已完成，后续 completion closeout 会处理完成状态。

**当前结论：** `docs/superpowers/specs/2026-05-25-mcp-management-surface-v0-design.md` 已定义 post-V1 MCP management surface：单一 Web management view、project-scoped connector list、tool / role / permission / approval summaries、deterministic/local connector health、safe read-only execution affordance、failure diagnostics 和 navigation re-entry。`docs/superpowers/plans/2026-05-25-mcp-management-surface-v0.md` 已把 Stage 51 implementation planning 收窄为 docs-only closeout plan，不修改 runtime、Web、backend、worker、MCP SDK、tool execution code 或 tests。该设计明确 MCP Management Surface 是 existing MCP registry / read-only execution / observation 的安全产品投影，不是新的 MCP SDK、write tool、worker execution 或 raw output 通道。

**后续收尾：**

- Stage 51 completion closeout 仍需创建 `docs/mcp-management-surface-v0-kickoff.md` 或同等 completion note，更新本 roadmap 把 Stage 51 标为 completed，并保持推荐队列为 Stage 54 default、Stage 48 conditional、Stage 50 optional、Stage 52 / Stage 53 discovery。
- 当前推荐实现路由切到 Stage 54 MCP Management Surface v0 Implementation。

**非目标：**

- 不实现 runtime、Web、backend、worker、MCP SDK 或 tool execution code。
- 不接入 remote MCP SDK、write tools 或 MCP worker execution。
- 不实现 auth/RBAC、secret storage、deployment/provider changes。
- 不把 MCP 加回 V1 alpha，不把 provider fallback、real deployment runner、browser platform 或 auth/storage 多方向混入 Stage 51 kickoff。

### Stage 54：MCP Management Surface v0 Implementation

**状态：** Stage 51 kickoff 后的当前默认推荐实现阶段。
