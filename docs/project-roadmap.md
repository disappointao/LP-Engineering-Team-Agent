...
- Task Intent Routing / AI Follow-ups v0：Stage59 已完成设计，建议作为当前 Web/agent 体验小阶段优先实现：普通聊天不显示推荐追问；LP 复杂任务才显示基于上下文生成的推荐追问；LP task 内每次输入先判定普通问答、继续执行、新复杂任务或澄清，避免把所有输入都当作 agent continuation。
...
- 新增 deterministic task intent router，区分普通问答、继续当前 LP、创建新 LP 和澄清问题。
...
- 2026-05-26 Stage59 Task Intent Routing / AI Follow-ups v0 设计已写入：`docs/superpowers/specs/2026-05-26-task-intent-routing-ai-followups-design.md` 定义 LP-only AI 推荐追问、普通聊天不显示推荐追问、LP task 内输入先判定 `chat_in_task` / `agent_continue` / `agent_new_task` / `clarify`，并保留普通聊天里 LP prompt 走现有 `fallback_required -> live task submit` 的路径。本阶段设计不改变 Planner / Builder / Reviewer / Deployer orchestration、worker queue、artifact workspace、真实 provider 或 interrupt 安全边界；实现阶段应优先用 deterministic router 保持 no-key gates 稳定，再考虑真实模型 opt-in 增强。当前推荐队列暂时把 Stage59 implementation 放在 Stage52 discovery 前，完成后回到 Stage52。
