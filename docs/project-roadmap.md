...
- Task Intent Routing / AI Follow-ups v0：Stage59 已完成设计和实施计划，正在作为当前 Web/agent 体验小阶段执行：普通聊天不显示推荐追问；LP 复杂任务才显示基于上下文生成的推荐追问；LP task 内每次输入先由 AI intent router 判定普通问答、继续执行、新复杂任务或澄清，deterministic router 只作为 no-key 测试和安全兜底，避免把所有输入都当作 agent continuation。
...
**状态：** 实施计划已写入，当前执行中的 Web/agent 体验小阶段。
