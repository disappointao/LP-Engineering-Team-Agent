...
- LP chain 的 no-refresh live timeline、artifact progress、普通聊天式 token streaming、tool-call conversion、MCP execution 和 usage/cost reporting 仍然后置；Stage 28 已完成 task-first fixed chain、继续编辑、previous artifact context、真实 Planner / Builder structured output 覆盖和 recovery 边界。
