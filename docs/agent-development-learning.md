...
Stage57 的补充学习点是把真实 provider 本地运行做成 operator-friendly preflight，而不是把它提升为默认 Agent readiness gate。`.env.real-provider.example` 和 `pnpm real-provider:doctor` 只能降低本机配置摩擦；是否真实调用仍由 `REAL_MODEL_RUNTIME=1`、project Models route 和本机 key 共同决定。缺 key 时应 skip 或 fail closed，不应触网、不应回退成假成功，也不应记录 secret、完整 base URL、raw provider response 或完整 artifact 内容。
...
- Stage57 Real Provider Local Run Polish v0 已实现：新增 `.env.real-provider.example`、`pnpm real-provider:doctor` 和 Models 本地运行 checklist，帮助 operator 用本机 `.env.local` 试跑真实 provider；它不是默认 readiness gate，不触网、不打印 key/base URL 值，也不把缺 key 场景回退成假成功。
