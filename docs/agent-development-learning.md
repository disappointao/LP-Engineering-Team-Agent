...
- 当前第一版可用闭环优先级已收敛为 Web/API 普通问答、LP 固定链路工作流、Skill 上下文/命令和流式反馈；Stage54 后 MCP management 已作为 post-V1 safe metadata/read-only surface 可见，但真实 MCP SDK / remote MCP server adapter、write tools、MCP worker execution、真实部署、auth/RBAC 和广义 tool execution 仍后置。
...
后来 LP fixed chain 也开始让 `runAgentStep()` 优先消费 `runtime.stream()`，但这里的 streaming 是 provider transport 和用户体感优化，不是新的事实边界。运行开始前会先保存 `run.started`，让 live task panel 能马上显示当前阶段；模型 delta 只在 runtime 内部被消费和聚合，不写入 run event、message 或 artifact；只有 terminal `model.completed` / `completed` result 进入原有 parse、policy validation、repair、artifact workspace 和 run finalization。OpenAI-compatible / Anthropic-compatible provider 如果在 stream 请求下返回普通 JSON，也会被 adapter 安全兼容成一次 bounded terminal response；超时、HTTP 5xx/429 和网络失败仍沿用两次 bounded retry 与脱敏 `model.retry.*` events。也就是说，LP 大任务可以减少“长时间没反馈”的感觉，但仍不能把局部 JSON、raw model output 或所谓思考过程当作可恢复事实。

这个边界同样约束 Manus-style 过程反馈：Web 可以从 safe run records、run events、artifact progress 和 snapshot 派生“理解需求、生成静态文件、检查页面质量、准备交付”一类用户语言时间线，也可以展示“上下文已装载、模型已响应、文件已生成”等 chip；但这些都是对 repository facts 的显示模型，不是 raw provider stream、raw tool output、完整 artifact 内容或真实 chain-of-thought。
...
Stage57 的补充学习点是把真实 provider 本地运行做成 operator-friendly preflight，而不是把它提升为默认 Agent readiness gate。`.env.real-provider.example` 和 `pnpm real-provider:doctor` 降低本机配置摩擦；本地 `pnpm dev` 现在会在 `REAL_MODEL_RUNTIME=1` 且 env profile ready 时自动创建 `Local Real Provider` 项目、provider 和 `assistant` / `planner` / `builder` / `reviewer` / `deployer` routes，让单用户第一版可以直接试普通聊天和 LP 链路。非流式真实 provider 调用默认 timeout 是 `LP_AGENT_MODEL_PROVIDER_TIMEOUT_MS=120000`，可显式调高到最多 `300000`；流式调用不能继续把这个短 timeout 当作整段总时长，因为复杂 LP 生成可能首 token 慢、总耗时长。当前流式 provider adapter 使用独立的 `firstByteMs`、`idleMs` 和 `maxDurationMs`：本地真实 runtime 默认首包 180s、空闲 180s、总上限 15 分钟，并可通过 `LP_AGENT_MODEL_PROVIDER_STREAM_FIRST_BYTE_TIMEOUT_MS`、`LP_AGENT_MODEL_PROVIDER_STREAM_IDLE_TIMEOUT_MS`、`LP_AGENT_MODEL_PROVIDER_STREAM_MAX_DURATION_MS` 覆盖。无论是哪种超时，最终仍必须形成 bounded `model_provider_request_timeout` 诊断和安全失败消息。这个自动引导仍只读本机 env、只保存 env var name 和 provider config，不保存真实 key；缺 key 时应 skip 或 fail closed，不应触网、不应回退成假成功，也不应记录 secret、完整 base URL、raw provider response 或完整 artifact 内容。
...
- Stage 28 LP Agent Chain End-to-End v0 已实现：Web LP 复杂任务采用 task-first fixed chain orchestration；Planner / Builder / Reviewer / Deployer 的 run、handoff、artifact workspace、deployment handoff 和 recovery facts 都绑定到同一个 task。`runAgentStep()` 已能优先消费 `runtime.stream()`，运行开始会先落 `run.started` 供 live task 观察，terminal result 仍回到 structured output parse / repair / policy 边界；Reviewer / Deployer 仍 deterministic / policy-driven。
...
- Stage 43 Run Timeline and Recovery UX Polish v0 的设计决策是 Web-only view-model polish：从现有 `LiveTaskStatePayload`、`RunLifecycleView`、safe run events 和 `recovery.runs` 派生 Planner / Builder / Reviewer / Deployer timeline、repair/retry hints、handoff/recovery hierarchy 和 transient progress affordance。学习重点是区分“事实”和“显示模型”：repository/run events/recovery contract 仍是事实源，UI 可以重组和强调这些事实，但不能把动画状态、推断的 pending step 或 query value 写回 agent state，也不能借 polish 暴露 raw provider response、raw tool output、完整 artifact 内容、本机路径或 secret。
- Stage 30 Skill-Only Alpha Hardening v0 已实现：它不是新增 Agent runtime 能力，而是把普通聊天 streaming、LP live task、artifact preview/export、项目 Skills、Skill command queue 和真实 provider opt-in 收敛成可交付的本地 alpha。学习重点是区分“第一版可用闭环的主路径”和“架构边界已存在但 alpha 不依赖的能力”：当时 MCP 页面只作为 deferred boundary copy 存在，V1 polished alpha 后进一步要求隐藏 MCP 可见入口；Stage 54 已把 MCP management 重新带回 post-V1 Web surface，但 remote MCP SDK、write tools、MCP worker execution、usage/cost reporting 和真实部署仍后置。Browser E2E 已由 Stage 31 补齐。默认 alpha 检查通过 `pnpm alpha:check` 运行 deterministic readiness gate，不触发真实 provider、MCP server、Postgres、Browser E2E 或真实部署。
...
- Stage 35 Provider Token Delta Streaming v0 已实现：普通聊天 `assistant` role 在真实 provider route 支持时会通过 `ModelGateway.stream()` 消费 provider SSE delta，Web/API 只展示 transient `assistant.delta`，最终事实仍是完整 assistant message 和 terminal run/model events；LP fixed chain 现在也可消费 `runtime.stream()`，但只把 terminal result 交给结构化 parse / repair / artifact policy，token delta 不成为 run event 或 artifact fact。
...
- Stage59 Task Intent Routing / AI Follow-ups v0 已实现：LP task 内每次输入先交给 AI intent router 判断 `chat_in_task` / `agent_continue` / `agent_new_task` / `clarify`，LP-only 推荐追问来自 bounded context 的 follow-up provider/cache；deterministic router 只用于 no-key gate、fake model fixture 和安全兜底。
...
- Stage59 implementation 已完成：AI intent router、LP-only AI 推荐追问、普通聊天无推荐追问，以及 LP task 内普通提问不启动 agent chain。Task 5 browser closeout 暴露 no-key Playwright path 的 LP follow-up suggestion fixture 仍需 controller 复核。
...
- 对已经 review passed、但没有 deployment / delivery artifact 的历史 LP task，不能永久视为 live。当前实现给最近有 run/message 活动的任务保留 2 分钟 handoff grace，避免刚通过 Reviewer 时提前停止；超过宽限期仍没有 deployment fact 时视为终态，停止 polling，避免旧任务在用户切换 task/project 时持续抢占状态接口。
...
- 终态判断应结合 run lifecycle、message boundary、artifact state 和最近活动时间。失败 run 可以终止 polling，用户请求后的无产物中间态不能终止 polling；但旧的 review-passed / no-deployment 状态也不能无限轮询，否则会制造后台噪音并拖慢 task switch 体感。
